...
client1.on("channelDelete", async channel => {
    ...
    let channel2 = client1.channels.cache.get(ayarlar1.log2)
...
client2.on("webhookUpdate", async (channel) => {
    ...
    channel.guild.channels.cache.get(ayarlar2.log3).send(`${entry.executor} tarafından sunucuda izinsiz webhook açıldı, webhook silinip ve banlandı!`)
...
client2.on("emojiDelete", async (emoji, message) => {
    ...
    let channel = client2.channels.cache.get(ayarlar2.log3)
